            ...
            <>
              <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="rounded-xl bg-slate-50 px-5 py-4">
                  <div className="text-sm text-slate-500">{isEdaMetric ? 'Stress range' : 'Range'}</div>
                  <div className="mt-2 text-4xl font-bold text-slate-900">
                    ...
                    {!isEdaMetric && <span className="ml-2 text-xl font-medium text-slate-500">{unit}</span>}
                  </div>
                </div>
                <div className="rounded-xl bg-slate-50 px-5 py-4">
                  <div className="text-sm text-slate-500">Latest</div>
                  <div className="mt-2 text-4xl font-bold text-slate-900">
                    {isEdaMetric ? (summary.latestLabel || '--') : (summary.latest != null ? summary.latest : '--')}
                    {!isEdaMetric && <span className="ml-2 text-xl font-medium text-slate-500">{unit}</span>}
                  </div>
                ...
                <div className="rounded-xl bg-slate-50 px-5 py-4">
                  <div className="text-sm text-slate-500">{metric === 'heartRate' ? 'Resting Heart Rate' : (isEdaMetric ? 'Dominant state' : 'Data points')}</div>
                  <div className="mt-2 text-4xl font-bold text-slate-900">
                    {metric === 'heartRate'
                      ? (summary.resting != null ? summary.resting : '--')
                      : (isEdaMetric ? (summary.dominantLabel || '--') : data.points.length)}
                    <span className="ml-2 text-xl font-medium text-slate-500">{metric === 'heartRate' ? unit : (isEdaMetric ? '' : 'records')}</span>
                  </div>
                </div>
              </div>
